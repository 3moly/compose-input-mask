...
class TransformedTextTest {
    ...
    @Test
    fun test_blank() {
        val transformedText = universalTransformation("", "", maskCharsToKeep = listOf())
        assertEquals("", transformedText.text.text)
    }

    @Test
    fun test_transformed_text() {
        val transformedText = universalTransformation("22335555", birthdayMask, maskCharToKeep = '.')
        assertEquals("22.33.5555", transformedText.text.text)
    }

    @Test
    fun test_transformed_text_filled() {
        val transformedText = universalTransformation("2233555566", birthdayMask, maskCharToKeep = '.')
        assertEquals("22.33.5555", transformedText.text.text)
    }

    @Test
    fun test_empty_mask_but_text() {
        val transformedText = universalTransformation("2233", "", maskCharToKeep = '.')
        assertEquals("", transformedText.text.text)
    }
    ...
    fun test_empty_mask_but_all_mask_chars_kept_2() {
        val visual = VisualTransformation {
            universalTransformation(it.text, birthdayMask, maskCharsToKeep = listOf('.'))
        }
